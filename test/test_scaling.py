...
def test_scale_inversion_polygon():
    inv_soln_archive_path = Path(
        Path(__file__).parent, 'fixtures/NZSHM22_InversionSolution-QXV0b21hdGlvblRhc2s6MTA3MDA2.zip'
    )
    scaled_soln_archive_path = Path(
        Path(__file__).parent, 'fixtures/NZSHM22_ScaledInversionSolution-QXV0b21hdGlvblRhc2s6MTEzMTA0.zip'
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with ZipFile(inv_soln_archive_path) as inv_soln_archive:
            with ZipFile(scaled_soln_archive_path) as scaled_soln_archive:
                ...
                scaled_rates_path = scaled_soln_archive.extract(
...
def test_scale_inversion():
    inv_soln_archive_path = Path(
        Path(__file__).parent, 'fixtures/NZSHM22_AveragedInversionSolution-QXV0b21hdGlvblRhc2s6MTA3MzE3.zip'
    )
    scaled_soln_archive_path = Path(
        Path(__file__).parent, 'fixtures/NZSHM22_ScaledInversionSolution-QXV0b21hdGlvblRhc2s6MTA3NjY4.zip'
    )
