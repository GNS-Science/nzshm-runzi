...
def test_run_executor(mocker, tmpdir):
    ...
    task.run(
        {
            ...
            "hazard_model": {"srm_logic_tree": srm_tree, "gmcm_logic_tree": gmcm_tree, "hazard_config": open_quake_config},
            "general": {"title": "the-title", "description": "the-description"},
        },
        {"task_id": "the-task-id",
         "use_api": True,
         "general_task_id": "the-general-task-id"},
    )
    ...
    assert task._toshi_api.openquake_hazard_task.create_input_variables["openquake_config"] == json.dumps(open_quake_config)
