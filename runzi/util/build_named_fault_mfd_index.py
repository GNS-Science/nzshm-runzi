...
class NamedFaultIndexBuilder:
                ...
                for filename in fnmatch.filter(files, pattern):
                    ...
                    if len(folder_path.parts) - len(PurePath(self._dir_name).parts) == 1:
                        ...
                        e.g
                        {
                            ...
                            'solution_file': (
                                                '/home/chrisbc/DEV/GNS/opensha-new/nshm-nz-opensha/src/'
                                                'python/automation/tmp/UnVwdHVyZUdlbmVyYXRpb25UYXNrOjE4M0FoblN5/
                                                'InversionSolution-RmlsZTo2-rnd0-t1380_RmlsZTo0ODMuMFN3cTRN.zip'
                                            ),
                        ...
                        solution_filepath = Path(
                            folder_path, '..', value['task_arguments']['file_id'], solution_name
                        ).resolve()
                        ...
                        info = dict(
                            ...
                            index_path=os.path.relpath(
                                PurePath(folder_path, "named_fault_mfds"), start=PurePath(self._dir_name)
                            ),
                        )
    ...
    def plot_div_template(self, title, path):
        return (
            f'<div id="{title}-inline_img" align="center"><a href="{path}" ><img src="{path}" alt="{title} '
            '"width="500" /></a><p><a href="#top">top</a></p></div>'
        )
    ...
    def sub_index_template(self, solution_info, mfd_folder='nucleation_cumulative'):
        ...
        list_insertion = "".join(
            [
                self.link_li_template(title, path)
                for (title, path) in self.build_image_index(
                    Path(self._dir_name, solution_info['index_path'], mfd_folder)
                )
            ]
        )
        img_insertion = "".join(
            [
                self.plot_div_template(title, path)
                for (title, path) in self.build_image_index(
                    Path(self._dir_name, solution_info['index_path'], mfd_folder)
                )
            ]
    ...
    def build_mfd_indexes(self, solution_info):
        ...
        links = [
            'nucleation_cumulative',
            'nucleation_incremental',
            'participation_cumulative',
            'participation_incremental',
        ]
