...
class BuilderTask:
    ...
    def run(self, task_arguments, job_arguments):
        ...
        if self.use_api:
            ...
            inversion_id = self._toshi_api.inversion_solution.upload_inversion_solution(
                task_id, filepath=result['solution'], meta=task_arguments, metrics=result['metrics']
            )
    ...
    def process(self, solution_id, solution_filepath, location, radius_m, rate_threshold):
        ...
        new_archive = PurePath(
            WORK_PATH, f"{location[0]}_ruptures_radius({radius})_rate_filter({rate_threshold})_sub_solution.zip"
        )
