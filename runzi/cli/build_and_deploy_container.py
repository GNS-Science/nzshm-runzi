...
def build_docker_image(
    ...
    image_tag = "runzi-build:latest"

    # No --no-cache: unchanged Java/OpenQuake layers are reused for fast rebuilds, while the
    # runzi layer is keyed on the resolved commit SHA (RUNZI_GITREF below), so it rebuilds
    # whenever the code changes. --pull keeps the base python image current.
    build_cmd = [
        "docker",
        "build",
        "--pull",
...
def build_and_deploy_container(
    ...
    local_image_tag = "runzi-build:latest"
    ...
    try:
        ...
        print(f"Image: {local_image_tag if skip_push else image_uri}")
        if not skip_push:
