...
default_submission_args = SubmissionArgs(
    ...
    # is at 32. Defaults to EC2 (not Fargate), which is safe because OpenQuake caps its worker pool to the
    # container's vCPU (ecs_vcpu, shipped as allocated_vcpu): without it, on EC2 the container sees the host's
    # cores and OOMs (#344, ADR-0012).
...
class OQHazardTask:
    def __init__(self, user_args: OQHazardArgs, runtime_args: TaskRuntimeArgs):
...
if __name__ == "__main__":
    ...
    task = OQHazardTask(user_args, runtime_args)
