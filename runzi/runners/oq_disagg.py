"""This module provides the runner class for running OQ disagg calculations."""
...
import runzi.execute.oq_disagg_task as task_module
...
from runzi.execute import ArgSweeper, OQDisaggArgs
from runzi.execute.oq_hazard_task import get_locations_from_file
...
class OQDisaggJobRunner(JobRunner):
    ...
    job_name = "Runzi-automation-oq-disagg"

    def __init__(self, argument_sweeper: ArgSweeper):
        """Initialize the OQDisaggJobRunner.
        ...
        self.argument_sweeper.prototype_args = cast(OQDisaggArgs, self.argument_sweeper.prototype_args)
        ...
        # disallow sweppping locations_file_id or locations_file
        if any([k in self.argument_sweeper.swept_args for k in ["locations_file_id", "locations_file", "locations"]]):
            raise ValueError("Cannot sweep locations, locations_file_id, or locations_file for disaggregation")
        if self.argument_sweeper.prototype_args.locations:
            locations = get_locations(self.argument_sweeper.prototype_args.locations)
            vs30s: list[int] = []
        else:
            locations, vs30s = get_locations_from_file(
                self.argument_sweeper.prototype_args.locations_file,
                self.argument_sweeper.prototype_args.locations_file_id,
            )
