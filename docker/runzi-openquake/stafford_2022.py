...
"""
    sigmoid1d(x, start, finish, centre, slope)

1D sigmoid function allowing a smooth transition from `start` to `finish`.
...
"""
    empirical_sigma(T, r, m, start = 0.4, finish = 0.15)
...
def _nl_sigma (C, ctx, ln_y_ref):
    vs = ctx.vs30.clip(-np.inf, 1130.0)
    NL = C['phi2'] * (np.exp(C['phi3'] * (vs - 360.0)) - np.exp(C['phi3'] * (1130.0 - 360.0))) * (np.exp(ln_y_ref) / (np.exp(ln_y_ref) + C['phi4']))
    return NL
...
class Stafford2022(GMPE):
    ...
    def __init__(self, mu_branch ='Central', sigma_branch = "Central", adjust_c1 = True, adjust_chm = True, adjust_c7 = True, adjust_cg1 = True, **kwargs):
        """
        Aditional parameter for epistemic central,
        lower and upper bounds.
        """
        super().__init__(mu_branch=mu_branch, sigma_branch = sigma_branch, adjust_c1 = adjust_c1, adjust_chm = adjust_chm, adjust_c7 = adjust_c7, adjust_cg1 = adjust_cg1, **kwargs)
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        for m, imt in enumerate(imts):
            if repr(imt) == "PGV":
                ...
                T = imt.period
                imt_mean, imt_sig, imt_tau, imt_phi = get_mean_stddevs(T, self.mu_branch, self.sigma_branch, self.adjust_c1, self.adjust_chm, self.adjust_c7, self.adjust_cg1,  self.COEFFS[imt], ctx)
                mean[m] = imt_mean
