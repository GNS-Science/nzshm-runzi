...
### Fixed
- `nzshm-model` minimum requirement raised to the current release, `>=0.15.5` (was `>=0.15.3`), which carries a fix runzi depends on. The lock already resolved 0.15.5; this makes the floor explicit so the fix is guaranteed rather than incidental.
- Bug in zipping OQ input files generated by `oq_opensha_convert_task`
- Env var documentation formatting
- Inversion reweight parameter is truly a boolean not bool | None
